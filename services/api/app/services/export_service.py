...
class PDFExportService:
    ...
    def generate_profit_loss_pdf(data: Dict[str, Any]) -> BytesIO:
        ...
        summary_data = [
            [
                ...
                f"฿{PDFExportService.format_thai_number(summary['total_revenue'])}",
            ],
            ["ต้นทุนขาย (COGS)", f"฿{PDFExportService.format_thai_number(summary['total_cogs'])}"],
            [
                "กำไรขั้นต้น (Gross Profit)",
                f"฿{PDFExportService.format_thai_number(summary['gross_profit'])}",
            ],
        ...
        detail_data = [
            ...
            [
                "  ยอดขาย",
                PDFExportService.format_thai_number(income_statement["revenue"]["total_sales"]),
            ],
            [
                "  ภาษีขาย (Output VAT)",
                PDFExportService.format_thai_number(income_statement["revenue"]["output_vat"]),
            ],
            ["ต้นทุนขาย", ""],
            [
                "  ต้นทุนสินค้า",
                PDFExportService.format_thai_number(
            ...
            [
                "  ภาษีซื้อ (Input VAT)",
                PDFExportService.format_thai_number(
                    income_statement["cost_of_goods_sold"]["input_vat_estimate"]
                ),
            ],
            [
                "กำไรขั้นต้น",
                PDFExportService.format_thai_number(income_statement["gross_profit"]["amount"]),
            ],
        ]
    ...
    def generate_vat_sales_pdf(data: Dict[str, Any]) -> BytesIO:
        ...
        summary_data = [
            ...
            [
                "มูลค่ารวม VAT",
                f"฿{PDFExportService.format_thai_number(summary['total_including_vat'])}",
            ],
        ]
        ...
        if details:
            ...
            detail_table = Table(
                detail_data,
                colWidths=[1 * inch, 1 * inch, 0.7 * inch, 1.2 * inch, 1 * inch, 1.3 * inch],
            )
            detail_table.setStyle(
                TableStyle(
                    [
            ...
            if len(data["details"]) > 50:
                ...
                story.append(
                    Paragraph(
                        f"* แสดง 50 รายการแรก จากทั้งหมด {len(data['details'])} รายการ",
                        styles["Italic"],
                    )
                )
    ...
    def generate_cogs_pdf(data: Dict[str, Any]) -> BytesIO:
        ...
        if details:
            ...
            if len(data["details"]) > 30:
                ...
                story.append(
                    Paragraph(
                        f"* แสดง 30 รายการแรก จากทั้งหมด {len(data['details'])} รายการ",
                        styles["Italic"],
                    )
                )
...
class ExcelExportService:
    ...
    def generate_profit_loss_excel(data: Dict[str, Any]) -> BytesIO:
        ...
        summary_data = [
            (
                "รายได้รวม (Total Revenue)",
                ExcelExportService.format_currency(summary["total_revenue"]),
            ),
            ("ต้นทุนขาย (COGS)", ExcelExportService.format_currency(summary["total_cogs"])),
            (
                "กำไรขั้นต้น (Gross Profit)",
                ExcelExportService.format_currency(summary["gross_profit"]),
            ),
        ...
        detail_data = [
            ...
            (
                "  ยอดขาย",
                ExcelExportService.format_currency(income_statement["revenue"]["total_sales"]),
            ),
            (
                "  ภาษีขาย (Output VAT)",
                ExcelExportService.format_currency(income_statement["revenue"]["output_vat"]),
            ),
            ("ต้นทุนขาย", ""),
            (
                "  ต้นทุนสินค้า",
                ExcelExportService.format_currency(
                    income_statement["cost_of_goods_sold"]["total_cogs"]
                ),
            ),
            (
            ...
            (
                "กำไรขั้นต้น",
                ExcelExportService.format_currency(income_statement["gross_profit"]["amount"]),
            ),
        ]
        ...
        for i, (label, value) in enumerate(detail_data):
            ...
            if i == 0:  # Header
                ...
                ws[f"A{row + i}"].fill = PatternFill(
                    start_color="D4F1D4", end_color="D4F1D4", fill_type="solid"
                )
                ws[f"B{row + i}"].fill = PatternFill(
                    start_color="D4F1D4", end_color="D4F1D4", fill_type="solid"
                )
    ...
    def generate_vat_sales_excel(data: Dict[str, Any]) -> BytesIO:
        ...
        for item in data["details"]:
            ...
            ws.cell(row=row, column=4).value = ExcelExportService.format_currency(
                item["price_before_vat"]
            )
            ws.cell(row=row, column=4).number_format = "#,##0.00"
            ws.cell(row=row, column=5).value = ExcelExportService.format_currency(
                item["vat_amount"]
            )
            ws.cell(row=row, column=5).number_format = "#,##0.00"
            ws.cell(row=row, column=6).value = ExcelExportService.format_currency(
                item["price_including_vat"]
            )
